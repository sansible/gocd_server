# You can overwrite any of the default variables in the `.make` file
include .make
...
# ensure virtualenv bin folder is on the PATH, and that sub-shells have also
# correct PATH.
...
tests/roles: .venv_$(ANSIBLE_INSTALL_VERSION)
	mkdir -p tests/roles
...
test_ansible: .venv_$(ANSIBLE_INSTALL_VERSION) tests/roles test_ansible_build test_ansible_configure
...
test_ansible_%: .venv_$(ANSIBLE_INSTALL_VERSION) tests/roles
	cd tests && ansible-playbook \
		--inventory inventory \
		--connection local \
...
vagrant: .venv_$(ANSIBLE_INSTALL_VERSION) tests/roles
...
# ensure empty .make file if it does not exist
.make:
	touch .make
